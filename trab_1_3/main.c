...
SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOX); // Substitua X pelo número da porta GPIO utilizada
GPIOPinTypeGPIOInput(GPIO_PORTX_BASE, GPIO_PIN_X); // Substitua X pelo número do pino utilizado

GPIOPinIntEnable(GPIO_PORTX_BASE, GPIO_PIN_X); // Substitua X pelo número da porta GPIO utilizada
GPIOPinIntClear(GPIO_PORTX_BASE, GPIO_PIN_X); // Substitua X pelo número do pino utilizado
GPIOPinIntEnable(GPIO_PORTX_BASE, GPIO_PIN_X); // Substitua X pelo número do pino utilizado
GPIOIntTypeSet(GPIO_PORTX_BASE, GPIO_PIN_X, GPIO_FALLING_EDGE); // Substitua X pelo número do pino utilizado e configure a borda de interrupção desejada

IntEnable(INT_GPIOX); // Substitua X pelo número da porta GPIO utilizada

void ButtonInterruptHandler(void)
{
    // Coloque o código para lidar com a interrupção dos botões aqui
}

GPIOIntRegister(GPIO_PORTX_BASE, ButtonInterruptHandler); // Substitua X pelo número da porta GPIO utilizada


void quadrado(uint8_t pino){
